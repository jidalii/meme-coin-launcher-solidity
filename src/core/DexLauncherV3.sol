...
import "./interfaces/IDexLauncherV3.sol";

contract DexLauncherV3 is IDexLauncherV3, Ownable, ReentrancyGuard {
        ...
        returns (address token0, uint256 amount0, address token1, uint256 amount1)
    {
        ...
        PositionInfo memory _positionInfo = _positionInfoForToken[tk];
        uint256 tokenId = _positionInfo.lpTokenId;
        token0 = _positionInfo.token0;
        token1 = _positionInfo.token1;
        ...
        // TODO: should I transfer NFT back to core contract?
    }
        ...
        returns (uint256 tokenId, uint128 liquidity, address token0, uint256 amount0, address token1, uint256 amount1)
    {
        ...
        _positionInfoForToken[tk] =
            PositionInfo({lpTokenId: tokenId, poolAddress: pool, token0: token0, token1: token1});
    ...
    function tokenIdFromToken(address tk) external view returns (uint256 tokenId) {
        return _positionInfoForToken[tk].lpTokenId;
    }
